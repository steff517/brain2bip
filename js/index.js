(function() {
    ...
    var longpassphraseChangeTimeoutEvent = null;
    ...
    DOM.longpassphrase=$(".longpassphrase");
    DOM.network = $(".network");
    DOM.bip32Client = $("#bip32-client");
    DOM.phraseNetwork = $("#network-phrase");
    DOM.useAdvanced = $(".use-advanced");
    DOM.entropyContainer = $(".entropy-container");
    DOM.advancedContainer = $(".advanced-container");
    DOM.entropy = $(".entropy");
    ...
    function setAdvancedVisibility() {
        if (isUsingAdvanced()) {
            DOM.advancedContainer.removeClass("hidden");
            phraseChanged();
        }
        else {
            DOM.advancedContainer.addClass("hidden");
        }
}

	function isUsingAdvanced() {
        return DOM.useAdvanced.prop("checked");
	}
	...
	function delayedLongpassphraseChanged() {
        hideValidationError();
        showPending();
        if (longpassphraseChangeTimeoutEvent != null) {
            clearTimeout(longpassphraseChangeTimeoutEvent);
        }
        longpassphraseChangeTimeoutEvent = setTimeout(longpassphraseChanged, 400);
    }
    
    function calcStrength() {
    	var result = zxcvbn(DOM.longpassphrase.val());
    	var strength='very strong';
    	if (result.guesses_log10<65 || result.crack_times_seconds.offline_fast_hashing_1e10_per_second<3e+55) {strength='very strong'; DOM.strength.css("color", "#42a118");}
    	if (result.guesses_log10<40 || result.crack_times_seconds.offline_fast_hashing_1e10_per_second<3e+30) {strength='strong'; DOM.strength.css("color", "#42a118");}
    	if (result.guesses_log10<25 || result.crack_times_seconds.offline_fast_hashing_1e10_per_second<3e+15) {strength='medium'; DOM.strength.css("color", "#caa111");}
    	if (result.guesses_log10<15 || result.crack_times_seconds.offline_fast_hashing_1e10_per_second<3e+7) {strength='weak'; DOM.strength.css("color", "#d73e0f");}
    	if (result.guesses_log10<9 || result.crack_times_seconds.offline_fast_hashing_1e10_per_second<3) {strength='very weak'; DOM.strength.css("color", "#d73e0f");}
		DOM.strength.html(strength);
